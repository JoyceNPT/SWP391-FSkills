...
//        String sql = "SELECT UserID, UserName, DisplayName, Role, BanStatus, AvatarGoogle FROM Users WHERE LOWER(UserName) LIKE LOWER(?)";
//        try ( PreparedStatement ps = conn.prepareStatement(sql)) {
...
//        List<User> us = new ArrayList<>();
//        String sql = "SELECT UserName, DisplayName, Email, Password, Role, DateOfBirth, UserCreateDate, info, BanStatus, PhoneNumber, AvatarGoogle FROM Users WHERE UserName = ?";
//        PreparedStatement ps = conn.prepareStatement(sql);
//        ps.setString(1, informUser);
//        ResultSet rs = ps.executeQuery();
//        while (rs.next()) {
...
//        List<User> list = new ArrayList<>();
//        String sql = "select UserID, UserName, DisplayName, Role, BanStatus from Users Order by ReportAmount Desc";
//        try {
//            PreparedStatement ps = conn.prepareStatement(sql);
//            ResultSet rs = ps.executeQuery();
//            while (rs.next()) {
